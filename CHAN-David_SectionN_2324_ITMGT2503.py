...
with open ("/Users/david/Desktop/2nd Year/Intersession/ITM 25.03/script.json" , "r") as file:
...
jsondata_string = (str(jsondata)).upper()
new = jsondata_string.replace("<P>","")
new2 = new.replace("</P>", "")
new3 = new2.replace("<B>","")
new4 = new3.replace("</B>","")
new5 = new4.replace("<I>","")
...
final_string = new6.replace(unnecessary_punctuation , "")

with open("/Users/david/Desktop/2nd Year/Intersession/ITM 25.03/output_dictionary_Q1.json" , "r") as file:
...
while i< len(wordcount_dictionary):
    wordcount_dictionary["word"] = output[i]
    wordcount_dictionary["count"] = len(output[i])
    i+=1
...
final_wordlist_of_tuples = [(word, count)]
...
def optimal_solution(num_fighters, num_rangers, num_clerics, max_Z):
    optimal_Z = max_Z
    optimal_Z = (10*num_fighters) + (12*num_rangers) + (16*num_clerics)
    optimal_F = optimal_Z - (12*num_rangers) - (16*num_clerics)
    optimal_R = optimal_Z - (10*num_fighters) - (16*num_clerics)
    optimal_C = optimal_Z - (10*num_fighters) - (12*num_rangers) 
        
    if (1000* num_fighters) + (600*num_rangers) + (750*num_clerics) > 18000:
        print("Too much Gold!")

    if (500*num_fighters)+(750*num_rangers)+(300*num_clerics) > 12000:
        print("Too much Lumber!")

    if (50*num_fighters)+(45*num_rangers)+(75*num_clerics) > 1500:
        print("Too much Food!")

    if num_fighters <0 and num_rangers <0 and num_clerics <0:
        print("No negatives allowed!")

    return "Optimal Fighters Points: " + str(optimal_F) + "; Optimal Rangers Points: " + str(optimal_R) + "; Optimal Clerics Points: "+ str(optimal_C) + "; Max Z: " + str(optimal_Z)

optimal_solution(4,5,6,332)
...
def Collatz_winner(number_list):
    ...
    for char in list:
        number_length = len(char[i])
        lengths.append(number_length)
        i += 1

        max_collatz = max(list)
        collatz_length = len(char)

        if char == max_collatz and char == collatz_length:
            winner = char

    return winner + " is the winner"
        
Collatz_winner("5, 16, 8, 4, 2, 1")
